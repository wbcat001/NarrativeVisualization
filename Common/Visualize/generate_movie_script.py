...
def generate_movie_script(text):
    ...
    STATEMENT: Actions or movements
    DIALOGUE: Spoken lines
    LOCATION: Scene location
    DEFAULT: Other descriptions

    When there is a change of location, insert a line with a LOCATION tag. LOCATION descriptions should be written in a hierarchical manner, connected by - if there is a hierarchy. Following this, use a STATEMENT tag to describe the situation or events in the new location. 

    If DIALOGUE is selected, include the name of the speaker in the format NAME: name.

    Select STATEMENT with appropriate frequency and add a description for the scene. Try to ensure that people unfamiliar with the story will be able to read and understand it.If possible, make it so that they can read it and picture it.

    

    It is not necessary to script all the text of the novel, but to describe the appropriate information as a visualisation. 

    In output text, avoid using smart quotes Instead, use standard single quotes (' ') or double quotes (" ") for consistency and compatibility with text processing systems."

    Please ensure that every entry includes both a Tag and a Content field!!!!!!
    ...
    pattern = r"Tag: (\w+),(?: Name: (\w+),)? Content: (.+)"
